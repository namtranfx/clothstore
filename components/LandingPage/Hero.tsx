...
const Hero: React.FC<HeroProps> = ({storeInfo, h, isHome}) => {
  ...
  return (
    <div className='font-DMSans relative items-center flex flex-col mt-12'>
        <img width={920} height={h} src={storeInfo.cover_image_url} alt='Hero banner' className='w-full rounded-xl'/>
        <div className='absolute top-[40%] text-white flex flex-col items-center'>
            <p className='md:text-[6rem] text-[1.75rem] font-light text-center'>{storeInfo.store_name}</p>
            {isHome && <Link href={"men-clothing"}>
